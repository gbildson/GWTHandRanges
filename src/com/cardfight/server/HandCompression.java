...
import com.cardfight.server.CalculateStats.RankTotals;
...
public class HandCompression {
		...
		for ( int i = 0; i < allHands.size(); i++ ) {
			...
			for ( int j = i+1; j < allHands.size(); j++ ) {
				...
				for ( int k = j+1; k < allHands.size(); k++ ) {
					...
					String uniqueHand = buildHand(hands, ordinals);
	...
	public static boolean hasOverlap(ArrayList<String> hands) {
	...
	public static String buildHand(ArrayList<String> hands, int[] ordinals) {
		HashMap<String, String> suits = new HashMap<String, String>();
		HashSet<String> used = new HashSet<String>();
		HashMap<String, String> pairs = new HashMap<String, String>();
		HashMap<String, Integer> notTogether = new HashMap<String, Integer>();
		...
		for (ID handID : handIDs) {
			...
			//System.out.println("mapSuits : " +before +" after "+suitNum);
		}
		debugStr += suitMap + "\n";
		//System.out.println("hand :"+ fullHand);
		for (ID handID : handIDs) {
			String sid = handID.toString();
			//System.out.println("listing: "+sid);
			uniqueHand += sid;
		}
		...
		// Gather any rearrangement of hands
		//int[] ords = new int[handIDs.size()];
		int cord = 0;
		for (ID handID : handIDs) {
			//System.out.println(handID);
			ordinals[cord++] += handID.ord;
		}
	...
	static class UnsuitedID extends ID {
		...
		public int mapSuits(int suitNum) {
			if ( suit1Originator ) {
				if ( suitID1 != suitNum ) {
					debugStr += "Unsuited suitMap1 : " +suitID1 + " " + suitNum+ " rank: "+rank1+"\n";
			...
			if ( suit2Originator ) {
				if ( suitID2 != suitNum ) {
					debugStr += "Unsuited suitMap2 : " +suitID2 + " " + suitNum+  " s1Orig: " + suit1Originator +" r:"+HandCompression.rank[12-rank2]+"\n";
	...
	static class SuitedID extends ID {
		...
		public int mapSuits(int suitNum) {
			if ( suitOriginator ) {
				if ( suitID != suitNum ) {
					debugStr += "Suited suitMap1 : " +suitID + " " + suitNum+ " rank1: "+rank1+ " rank2: " +rank2+"\n";
					suitMap.put(""+suitID,""+suitNum++);
				} else {
					suitNum++;
				}
			}
